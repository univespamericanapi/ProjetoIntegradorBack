...
const novoDesfile = async (req, res) => {
	...
	if (!desfDados.desf_event) {
		res.status(400).send({
			message: 'Não foram enviadas todas as informações!',
		});
		return;
	}
...
const verificaConfigs = async (idEvento) => {
	...
	if (contagem >= configs.config_limit_inscr) {
		retorno.status = 201;
		retorno.message =
			'As vagas para o concurso acabaram, sua inscrição foi registrada na fila de espera!';
		return retorno;
	}
